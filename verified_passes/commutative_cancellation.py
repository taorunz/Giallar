...
from giallar.core.impl.qcircuit import empty_circuit, commutation
...
class CommutativeCancellation(Transformation):
    ...
    def run(self, input_prop):
        ...
        for block in circ.blocks(commutation):
            if block.pairwiseCommute():
                block = block.cancelGates()
            out_circ.extend(block)
