...
from giallar.core.impl.qcircuit import empty_circuit
...
class CXCancellation(Transformation):
    ...
    def run(self, input_prop):
        ...
        while gates_remaining.size() > 0:
            if gates_remaining[0].isCXGate():
                if gates_remaining.next_gate(0) == gates_remaining[0]:
                    gates_remaining = gates_remaining.cancelStartingCX()
                else:
